...
@if (isset($header))
    ...
    @section('top_meta_tags')
        <meta name="description" content="{!! str_limit(strip_tags($header->meta_description ?? $setting->description), 160, ' ...') !!}">
...
@section('content')
    ...
    <main class="contact-us">
        <section class="contact-us-hero">
            <div class="container">
                <div class="contact-us-crumb"><a href="{{ route('home') }}"
                        wire:navigate>{{ __('navbar.home') }}</a><span>/</span>{{ __('navbar.contact') }}</div>
                <div class="contact-us-hero-grid">
                    <div>
                        <div class="contact-us-tag" style="color: #ffffff"><i class="fa-solid fa-circle"></i> Let's connect
                        </div>
                        <h1>Contact Us</h1>
        ...
        <section class="contact-us-routes">
            <div class="container contact-us-routes-grid">
                @if (isset($setting->email_one))
                    <a class="contact-us-route" href="mailto:{{ $setting->email_one }}"><i
                            class="bi bi-envelope-arrow-up"></i>
                        <div><small>Email us</small><strong>{{ $setting->email_one }}</strong></div>
                    </a>
                    @endif @if (isset($setting->phone_one))
                        <a class="contact-us-route" href="tel:{{ $setting->phone_one }}"><i class="bi bi-telephone"></i>
                            <div><small>Call us</small><strong>{{ $setting->phone_one }}</strong></div>
                        </a><a class="contact-us-route"
                            href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $setting->phone_one) }}" target="_blank"
                            rel="noopener"><i class="bi bi-whatsapp"></i>
                    ...
                    <div class="contact-us-form">
                        ...
                        @if (Session::has('success'))
                            <div class="contact-us-alert"><button type="button" class="contact-us-alert-close"
                                    onclick="this.parentElement.style.display='none'">&times;</button>{{ Session::get('success') }}
                            </div>
                            @endif @if (Session::has('error'))
                                <div class="contact-us-alert contact-us-alert-error"><button type="button"
                                        class="contact-us-alert-close"
                                        onclick="this.parentElement.style.display='none'">&times;</button>{{ Session::get('error') }}
                                </div>
                            @endif
                            @if ($errors->any())
                            ...
                            <form method="post" action="{{ route('contact.send') }}" accept-charset="utf-8">@csrf<div
                                    ...
                                    <div class="contact-us-field"><label
                                            for="contact-us-name">{{ __('contact.your_name') }}
                                            <b>*</b></label><input id="contact-us-name" type="text" name="name"
                                            value="{{ old('name') }}" placeholder="Jane Doe" required></div>
                                    <div class="contact-us-field"><label
                                            for="contact-us-phone">{{ __('contact.phone_no') }}</label><input
                                            id="contact-us-phone" type="text" name="phone" value="{{ old('phone') }}"
                                            placeholder="+880 1XXX-XXXXXX"></div>
                                    <div class="contact-us-field"><label
                                            for="contact-us-email">{{ __('contact.email_address') }}
                                            <b>*</b></label><input id="contact-us-email" type="email" name="email"
                                            value="{{ old('email') }}" placeholder="you@company.com" required></div>
                                    <div class="contact-us-field"><label
                                            for="contact-us-subject">{{ __('contact.subject') }}
                                            <b>*</b></label><select id="contact-us-subject" name="subject" required>
                                            <option value="" disabled {{ old('subject') ? '' : 'selected' }}>Select a
                                                topic</option>
                                            <option value="Web Development"
                                                {{ old('subject') == 'Web Development' ? 'selected' : '' }}>Web
                                                Development
                                            </option>
                                            <option value="Laravel Development"
                                                {{ old('subject') == 'Laravel Development' ? 'selected' : '' }}>
                                                Laravel
                                                Development</option>
                                            <option value="Mobile App Development"
                                                {{ old('subject') == 'Mobile App Development' ? 'selected' : '' }}>
                                                Mobile App Development</option>
                                            <option value="SEO & Marketing"
                                                {{ old('subject') == 'SEO & Marketing' ? 'selected' : '' }}>SEO &amp;
                                                Marketing</option>
                                    ...
                                    <div class="contact-us-field contact-us-full"><label
                                            for="contact-us-message">{{ __('contact.your_massage') }} <b>*</b></label>
                                        <textarea id="contact-us-message" name="message"
                                            placeholder="Tell us about your project, goals, and expected timeline..." required>{{ old('message') }}</textarea>
                                    </div>
                                </div>
                                <div class="contact-us-submit-row"><button
                                    ...
                                    <p class="contact-us-note">We only use your details to respond to your enquiry. Your
                                        information stays
                                        private.</p>
                                </div>
                            </form>
        ...
        <section class="contact-us-process">
            <div class="container contact-us-process-grid">
                <div class="contact-us-process-copy">
                    ...
                    <p>No sales pressure or vague replies. We learn about the opportunity and give you useful, honest
                        direction.</p>
                </div>
        ...
        <section class="contact-us-faq">
            <div class="container contact-us-faq-wrap">
                ...
                <div class="contact-us-faq-item"><button class="contact-us-faq-q"
                        type="button"><small>01</small><strong>How soon
                            will I hear back?</strong><i class="fa-solid fa-plus"></i></button>
                    <div class="contact-us-faq-a">
                        <p>Most enquiries receive a reply within 24 hours on business days, with a clear recommendation for
                            the next
                            step.</p>
                    </div>
                </div>
                <div class="contact-us-faq-item"><button class="contact-us-faq-q"
                        type="button"><small>02</small><strong>Do you
                            work with startups and small teams?</strong><i class="fa-solid fa-plus"></i></button>
                    <div class="contact-us-faq-a">
                        <p>Yes. We work with early-stage startups as well as established organisations, tailoring the
                            approach to your
                            needs.</p>
                    </div>
                </div>
                <div class="contact-us-faq-item"><button class="contact-us-faq-q"
                        type="button"><small>03</small><strong>Can we
                ...
                <div class="contact-us-faq-item"><button class="contact-us-faq-q"
                        type="button"><small>04</small><strong>What do
                            you need to prepare a quote?</strong><i class="fa-solid fa-plus"></i></button>
                    <div class="contact-us-faq-a">
                        <p>A short outline of the project, desired result, timeline, and budget range is enough to begin.
                        </p>
                    </div>
                </div>
            </div>
                ...
                <div class="container">
                    ...
                    <div class="contact-us-logos">
                        @foreach ($clients as $client)
                            <div class="contact-us-logo"><img src="{{ asset('uploads/client/' . $client->image_path) }}"
                                    alt="{{ $client->title }}"></div>
                        @endforeach
                    </div>
                </div>
            </section>
        ...
        <section class="contact-us-cta">
            <div class="container">
                ...
                <p>Book a short call and speak with our team directly.</p><button id="open-modal"
                    class="contact-us-book google-meet-button">Book a meeting <i
    ...
    <script>
        function initContactFaq() {
            document.querySelectorAll('.contact-us-faq-q').forEach(function(b) {
                b.addEventListener('click', function() {
                    var item = b.parentElement,
                        open = item.classList.contains('contact-us-open');
                    document.querySelectorAll('.contact-us-faq-item.contact-us-open').forEach(function(x) {
                        x.classList.remove('contact-us-open')
                    });
                    if (!open) item.classList.add('contact-us-open')
                })
            })
        }

        // wire:navigate দিয়ে contact পেজে ফিরে এলে FAQ এলিমেন্টগুলো নতুন করে DOM-এ বসে,
        // তাই দুটো ইভেন্টেই bind করা হয়েছে
        document.addEventListener('DOMContentLoaded', initContactFaq);
        document.addEventListener('livewire:navigated', initContactFaq);
    </script>
@endsection
