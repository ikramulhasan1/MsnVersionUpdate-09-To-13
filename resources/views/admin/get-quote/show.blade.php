                    ...
                    <div class="card-body">
                                        ...
                                        <td class="row">: 
                                            {{-- <a href="{{ asset('uploads/quote/' . $row->file_path) }}" target="_blank"
                                                ...
                                                @foreach(explode(',', $row->file_path) as $file)
                                                    ...
                                                    <a class="m-1" href="{{ asset('uploads/quote/' . $file) }}" target="_blank"><span
                        ...
                        @if(isset($row->message))
                            <p><span class="text-highlight">{{ __('dashboard.description') }}: </span>
                                {!! strip_tags($row->message, '<p><a><b><i><u><strong><br><ul><ol><li><del><ins><sup><sub><pre>') !!}
                            </p>
                            <hr />
                        @endif
                    </div>
                    ...
                    <div class="card-body">
                        ...
                        <p><span class="text-highlight">{{ __('dashboard.prefer_contact') }} </span>
                            @if($row->prefer_contact == 1)
                                <span>{{ __('dashboard.phone') }}: <a href="tel:{{ $row->phone }}"
                                        target="_blank">{{ $row->phone }}</a></span>
                            @elseif($row->prefer_contact == 2)
                                <span>{{ __('dashboard.email') }}: <a href="mailto:{{ $row->email }}"
                                        target="_blank">{{ $row->email }}</a></span>
                            @endif
                        </p>
                        ...
                        @if(isset($template_estimated))
                            <a href="{{ route($route . '.invoice', ['id' => $row->id, 'action' => 'estimated']) }}"
                        ...
                        @if(isset($template_approved))
                            <button type="button" class="btn btn-success btn-sm mb-1" data-toggle="modal"
                                data-target="#approveModal-{{ $row->id }}">
            ...
            <div class="row">
                <div class="col-12 col-lg-12">
                    ...
                    @if(isset($template_invoice))
                        <a href="{{ route($route . '.invoice', ['id' => $row->id, 'action' => 'invoice']) }}"
                                    ...
                                    @foreach($row->invoices as $key => $row)
                                        <tr>
                                            ...
                                            <td>
                                                @if($row->status == 1)
                                                    <span
                                                        class="badge badge-primary badge-pill">{{ __('dashboard.pending') }}</span>
                                                @elseif($row->status == 2)
                                                    <span class="badge badge-success badge-pill">{{ __('dashboard.paid') }}</span>
                                                @elseif($row->status == 0)
                                                    <span
                                                        class="badge badge-danger badge-pill">{{ __('dashboard.canceled') }}</span>
                                                @endif
                                            </td>
                                        </tr>
                                    @endforeach
