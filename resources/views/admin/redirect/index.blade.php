...
<div class="container-fluid">
    ...
    <div class="row">
        <div class="col-12">
            <a href="{{ route('admin.redirects.create') }}" class="btn btn-primary">{{ __('dashboard.add_new') }}</a>

            <a href="{{ route('admin.redirects.index') }}" class="btn btn-info">{{ __('dashboard.refresh') }}</a>
        </div>
    </div>
                    ...
                    <table id="basic-datatable" class="table table-striped table-hover table-dark nowrap full-width">
                        ...
                        <tbody>
                          @foreach( $rows as $key => $row )
                            <tr>
                                <td>{{ $key + 1 }}</td>
                                <td>{!! str_limit(strip_tags($row->submitted_url), 50, ' ...') !!}</td>
                                <td>{{ $row->redirect_to }}</td>
                                {{-- <td>
                                    @if( $row->status == 1 )
                                ...
                                <td>
                                    <a href="{{ route('admin.redirects.show', [$row->id]) }}" class="btn btn-success btn-sm">
                                        <i class="fas fa-eye"></i>
                                    </a>

                                    <a href="{{ route('admin.redirects.edit', [$row->id]) }}" class="btn btn-primary btn-sm">
                                    ...
                                    <div class="modal fade" id="deleteModal-{{ $row->id }}" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
                                        ...
                                        <form action="{{ route('admin.redirects.destroy', [$row->id]) }}" method="post" class="delete-form">
                                        @csrf
                                        @method('DELETE')
