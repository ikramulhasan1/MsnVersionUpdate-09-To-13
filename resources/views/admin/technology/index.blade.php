...
<div class="container-fluid">
    ...
    <div class="row mt-3">
        <div class="col-12">
            <a href="{{ route('admin.technologies.create') }}" class="btn btn-primary">{{ __('dashboard.add_new') }}</a>

            <a href="{{ route('admin.technologies.index') }}" class="btn btn-info">{{ __('dashboard.refresh') }}</a>
        </div>
    </div>
                        ...
                        <tbody>
                          @foreach( $rows as $key => $row )
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('admin.technologies.show', [$row->id]) }}" class="btn btn-success btn-sm">
                                        <i class="fas fa-eye"></i>
                                    </a>

                                    <a href="{{ route('admin.technologies.edit',[$row->id]) }}" class="btn btn-primary btn-sm">
                                    ...
                                    <div class="modal fade" id="deleteModal-{{ $row->id }}" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
                                        ...
                                        <form action="{{ route('admin.technologies.destroy', [$row->id]) }}" method="post" class="delete-form">
                                        @csrf
                                        @method('DELETE')
