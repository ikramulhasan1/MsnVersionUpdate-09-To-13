...
final class BusinessSignalsDetector
{
    ...
    {
        [$careersDetected, $careersDetail] = $this->detectCareers($crawledPages);
        [$hiringDetected, $hiringDetail] = $this->detectHiring($crawledPages);
        [$blogUpdateDetected, $blogUpdateDetail] = $this->detectBlogUpdate($crawledPages);
    ...
    private function detectHiring(array $crawledPages): array
    {
        foreach ($crawledPages as $crawledPage) {
            ...
            if ($jobPostingCount > 0) {
                return [true, "{$jobPostingCount} JobPosting schema block(s) found on {$crawledPage->url}"];
            }
        }

        return [false, 'A careers page may exist, but no schema.org JobPosting block was found confirming an active, specific listing.'];
    }
    ...
    private function detectBlogUpdate(array $crawledPages): array
    {
        ...
        foreach ($crawledPages as $crawledPage) {
            ...
            if ($publishedAt !== null && $publishedAt >= $cutoff) {
                ...
                return [true, "Blog/news page at {$crawledPage->url} has a schema.org publish/modified date of {$formatted}, within the last ".self::BLOG_FRESHNESS_WINDOW_DAYS.' days.'];
            }
        }

        if ($blogPageFound) {
            return [false, 'A blog/news section was found, but no schema.org date within the last '.self::BLOG_FRESHNESS_WINDOW_DAYS.' days could be confirmed.'];
        }
