<?php
return [
    'general' => [
        // ['name' => 'Python', 'icon' => '//cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg', 'url' => ''],
        // ['name' => 'Java', 'icon' => '//cdn.jsdelivr.net/gh/devicons/devicon/icons/java/java-original.svg', 'url' => ''],
        ['name' => 'JavaScript', 'icon' => '//cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg', 'url' => ''],
        // ['name' => 'C#', 'icon' => '//cdn.jsdelivr.net/gh/devicons/devicon/icons/csharp/csharp-original.svg', 'url' => ''],
        // ['name' => 'C++', 'icon' => '//cdn.jsdelivr.net/gh/devicons/devicon/icons/cplusplus/cplusplus-original.svg', 'url' => ''],
        ['name' => 'PHP', 'icon' => '//cdn.jsdelivr.net/gh/devicons/devicon/icons/php/php-original.svg', 'url' => ''],
        // ['name' => 'Ruby', 'icon' => '//cdn.jsdelivr.net/gh/devicons/devicon/icons/ruby/ruby-original.svg', 'url' => ''],
        // ['name' => 'Kotlin', 'icon' => '//cdn.jsdelivr.net/gh/devicons/devicon/icons/kotlin/kotlin-original.svg', 'url' => ''],
        // ['name' => 'Swift', 'icon' => '//cdn.jsdelivr.net/gh/devicons/devicon/icons/swift/swift-original.svg', 'url' => ''],
    ],
    'datasci' => [
        ['name' => 'R', 'icon' => '//cdn.jsdelivr.net/gh/devicons/devicon/icons/r/r-original.svg', 'url' => ''],
        ['name' => 'TensorFlow', 'icon' => '//cdn.jsdelivr.net/gh/devicons/devicon/icons/tensorflow/tensorflow-original.svg', 'url' => ''],
        ['name' => 'PyTorch', 'icon' => '//cdn.jsdelivr.net/gh/devicons/devicon/icons/pytorch/pytorch-original.svg', 'url' => ''],
        ['name' => 'Python', 'icon' => '//cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg', 'url' => ''],
    ],
    'mobile' => [
        ['name' => 'Flutter', 'icon' => '//cdn.jsdelivr.net/gh/devicons/devicon/icons/flutter/flutter-original.svg', 'url' => ''],
        ['name' => 'React Native', 'icon' => '//cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg', 'url' => ''],
        ['name' => 'Swift', 'icon' => '//cdn.jsdelivr.net/gh/devicons/devicon/icons/swift/swift-original.svg', 'url' => ''],
        ['name' => 'Kotlin', 'icon' => '//cdn.jsdelivr.net/gh/devicons/devicon/icons/kotlin/kotlin-original.svg', 'url' => ''],
    ],
    // React Ecosystem
    'react' => [
        ['name' => 'React.js', 'icon' => '//cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg', 'url' => ''],
        ['name' => 'Next.js', 'icon' => '//cdn.jsdelivr.net/gh/devicons/devicon/icons/nextjs/nextjs-original.svg', 'url' => ''],
    ],

    // Vue Ecosystem
    'vue' => [
        ['name' => 'Vue.js', 'icon' => '//cdn.jsdelivr.net/gh/devicons/devicon/icons/vuejs/vuejs-original.svg', 'url' => ''],
        // ['name' => 'Nuxt.js', 'icon' => '//cdn.jsdelivr.net/gh/devicons/devicon/icons/nuxtjs/nuxtjs-original.svg', 'url' => ''],
    ],

    // Node Ecosystem
    'node' => [
        ['name' => 'Node.js', 'icon' => '//cdn.jsdelivr.net/gh/devicons/devicon/icons/nodejs/nodejs-original.svg', 'url' => ''],
        ['name' => 'Express.js', 'icon' => '//cdn.jsdelivr.net/gh/devicons/devicon/icons/express/express-original.svg', 'url' => ''],
        ['name' => 'NestJS', 'icon' => '//nestjs.com/logo.svg', 'url' => ''], // NestJS doesn’t exist in Devicon, so custom logo
    ],

    // Laravel Ecosystem
    'laravel' => [
        ['name' => 'Laravel', 'icon' => '//upload.wikimedia.org/wikipedia/commons/thumb/9/9a/Laravel.svg/1154px-Laravel.svg.png', 'url' => ''],
        // ['name' => 'Symfony', 'icon' => '//cdn.jsdelivr.net/gh/devicons/devicon/icons/symfony/symfony-original.svg', 'url' => ''],
    ],
    'Content Management Systems' => [
        ['name' => 'WordPress', 'icon' => '//upload.wikimedia.org/wikipedia/commons/thumb/0/09/Wordpress-Logo.svg/2048px-Wordpress-Logo.svg.png', 'url' => ''],
        // ['name' => 'Symfony', 'icon' => '//cdn.jsdelivr.net/gh/devicons/devicon/icons/symfony/symfony-original.svg', 'url' => ''],
    ],
    'E-Commerce Solutions' => [
        ['name' => 'BigCommerce', 'icon' => '//www.code-mage.com/media/06/72/22/1595065961/bigcommerce-platform-logo.svg', 'url' => ''],
        ['name' => 'Shopify', 'icon' => '//www.realisable.co.uk/wp-content/themes/realisable/images/logos/shopify.svg', 'url' => ''],
        ['name' => 'WooCommerce', 'icon' => '//connectif.ai/wp-content/uploads/2023/07/woocommerce.png', 'url' => ''],
        ['name' => 'Wix', 'icon' => '//upload.wikimedia.org/wikipedia/commons/thumb/7/76/Wix.com_website_logo.svg/1280px-Wix.com_website_logo.svg.png', 'url' => ''],
    ],
];
